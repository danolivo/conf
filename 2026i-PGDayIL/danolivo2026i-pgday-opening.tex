...
\title[When Misestimation Inflates Your Cloud Bill]{When the Planner's Misestimation Inflates Your Cloud Bill}
...
\begin{tcolorbox}[enhanced, arc=3pt, colback=infobg, colframe=infoframe,
  boxrule=0.4pt, left=6pt, right=6pt, top=5pt, bottom=5pt,
  before skip=2pt, after skip=0pt]
{\scriptsize\color{ptoink}\textbf{A lot} of this query's time
went into rows a join built and then threw away.}
\end{tcolorbox}
\end{tcolorbox}
\end{column}
...
\newcommand{\lever}[3]{%
  ...
  \vspace{1pt}
  \noindent\hspace{0.8cm}{\small\itshape\textcolor{black!55}{#3}}\par
  \vspace{7pt}}
...
\lever{2}{Selectivity of scans and joins (rows removed by filter)}
...
\begin{forest} plangraph, for tree={font=\sffamily\tiny, inner xsep=2pt, l sep=7pt, s sep=4pt}
[Hash Join, rows={20}{20000}
  [Seq Scan, rows={100000}{100000}, flow=100000]
  [Hash, rows={1}{1000}, flow=1000
    [Nested Loop, detail={Removed: 4\,999\,000}, rows={1}{1000}, bad, flow=1000
      [Seq Scan, rows={1}{1000}, flow=1000]
      [Seq Scan, rows={5000}{5000}, loops={1000}, flow=5000000]
    ]
  ]
]
\end{forest}
...
\begin{forest} plangraph, for tree={font=\sffamily\tiny, inner xsep=2pt, l sep=7pt}
[HashAggregate, rows={2000}{100}, bad
  [Seq Scan, rows={20000}{20000}, flow=20000]
]
\end{forest}
...
\begin{tcolorbox}[enhanced, arc=3pt, colback=ptoaccent, colframe=ptoaccent,
  ...
  before skip=2pt, after skip=0pt]
\begin{center}
{\small\color{ptopaper} We should estimate optimisation potential of very different queries }
\end{center}
\end{tcolorbox}
\end{frame}
...
\begin{forest} plangraph, for tree={font=\sffamily\tiny, inner xsep=1.5pt, inner ysep=1.5pt, l=0pt, l sep=7pt, s sep=2pt}
[{\nd{Result}{\$1}{20000}}, bad
  [{\nd{Hash Join}{}{20000}}, flow=20000
    [{\nd{Seq Scan}{payments}{100000}}, flow=100000]
    [{\nd{Hash}{}{1000}}, flow=1000
      [{\nd{Hash Join}{}{1000}}, flow=1000
        [{\nd{Seq Scan}{orders}{1000}}, flow=1000]
        [{\nd{Hash}{}{5000}}, flow=5000
          [{\nd{Seq Scan}{customers}{5000}}, flow=5000]
        ]
      ]
    ]
  ]
...
\vspace{-0.3em}
\begin{sql}[fontsize=\tiny]
ANALYZE t;                               -- 40 times, the data never change
EXPLAIN SELECT * FROM t WHERE x = 77;    -- actual: 10000 rows every time
\end{sql}
\vspace{0.6em}
\begin{columns}[T,onlytextwidth]
...
\begin{column}{0.48\textwidth}
\begin{tcolorbox}[enhanced, arc=3pt, colback=paper, colframe=crash,
  boxrule=0.6pt, left=8pt, right=8pt, top=6pt, bottom=6pt,
  before skip=0pt, after skip=0pt, equal height group=stoch]
\small\color{ptoink}
...
\begin{tcolorbox}[enhanced, arc=3pt, colback=ptoaccent, colframe=ptoaccent,
  ...
  before skip=2pt, after skip=0pt]
\begin{center}
{\small\color{ptopaper}Statistics are a random sample: on unchanged data,\\
a fresh ANALYZE moves the estimate --- and flips the plan.}
\end{center}
\end{tcolorbox}

{\tiny\color{black!40}5\,M rows: 150 values $\times$ 10\,000 rows compete for 100 MCV slots,
 plus a long tail of rare values}
\end{frame}
...
\newcommand{\cardbody}[1]{\parbox[t][3.25cm][t]{4.8cm}{\raggedright\hyphenpenalty=10000 #1}}
\tikzset{
...
\begin{frame}\frametitle{Ratios, and how they spread}
\centering
\begin{tikzpicture}

  % the three nuances, one per previous slide
  \node[chip] (n1) at (-3.7, 0) {\textcolor{ptoaccent}{\bfseries 1}\enspace shapes and scales differ};
  \node[chip] (n2) at ( 0.0, 0) {\textcolor{ptoaccent}{\bfseries 2}\enspace nodes may not run};
  \node[chip] (n3) at ( 3.7, 0) {\textcolor{ptoaccent}{\bfseries 3}\enspace estimates drift};

  % the two parts of the answer
  \node[card=ptoink] (dim) at (-2.75, -0.75) {\cardbody{%
    {\small\bfseries Dimensionless metrics}\par\medskip
    per plan node:\par\smallskip
    \centerline{$e = \left|\,\ln\dfrac{\textit{actual}}{\textit{estimated}}\,\right|$}\par\smallskip
    averaged over the nodes that \emph{ran}; weights: none, square, share of time, share of cost\par\smallskip
    {\ttfamily avg\quad rms\quad twa\quad wca}}};

  \node[card=ptoaccent] (rst) at (2.75, -0.75) {\cardbody{%
    {\small\bfseries \texttt{rstats} type}\par\medskip
    a distribution, not a number:\par\smallskip
    \centerline{\ttfamily (count, mean, min, max, stddev)}\par\smallskip
    updated in place on every execution (Welford), $O(1)$ memory\par\smallskip
    wide \texttt{min..max} or large \texttt{stddev}: the plan is unstable, not just wrong}};

  \draw[link] (n1.south) -- (dim.north -| n1.south);
  \draw[link] (n2.south) to[out=-90, in=90] ([xshift=1.6cm]dim.north);
  \draw[link] (n3.south) -- (rst.north -| n3.south);
\end{tikzpicture}
...
\begin{tcolorbox}[enhanced, arc=3pt, colback=ptoaccent, colframe=ptoaccent,
  ...
  before skip=2pt, after skip=0pt]
\begin{center}
{\small\color{ptopaper}Any two queries become comparable ---\\
and each one tells you how steady it is.}
\end{center}
\end{tcolorbox}
\end{frame}
