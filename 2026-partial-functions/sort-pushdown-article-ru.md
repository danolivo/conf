...
(заимствуя термин из математики): оно вызывает ошибку, если входные данные не являются допустимым целым числом (например, `'42.1'`). Волатильность ничего об этом не говорит. Оптимизатор считает приведение свободно перемещаемым — и может протолкнуть его туда, где оно встретит данные, которые исходный план отфильтровал бы.
...
Александру Симонову (Тантор Лабс) — за идею проекта. Claude (Anthropic) — за помощь в подготовке текста публикации.
